        ...
        when(event){
            ...
            is CommentsEvent.GetAllInfo -> {
                viewModelScope.launch(Dispatchers.IO){
                    try {
                        val status = getApplicationStatusUseCase.invoke(state.value.applicationId).status
                        _state.value = state.value.copy(
                ...
                viewModelScope.launch(Dispatchers.IO) {
                    try {
                        if(state.value.applicationStatus != state.value.currentApplicationStatus){
                            updateApplicationStatusUseCase.invoke(applicationId = state.value.applicationId, status = state.value.applicationStatus)
                            _state.value = state.value.copy(
                                isComplete = true
                            )
                        }
